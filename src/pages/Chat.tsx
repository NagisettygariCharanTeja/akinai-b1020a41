...
const Chat = () => {
  ...
  return <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900' : 'bg-gradient-to-br from-gray-50 via-white to-gray-100'}`}>
      
      <div className="container mx-auto max-w-7xl px-4 py-4">
        {/* Header */}
        <motion.div className="flex justify-between items-center mb-6" initial={{
        y: -20,
        opacity: 0
      }} animate={{
        y: 0,
        ...
        ease: "easeOut"
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${isDarkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600'}`}>
              PREMIUM
            </span>
          </div>
          
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="sm" onClick={() => setSettingsOpen(true)} className={isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-700' : 'text-gray-600 hover:bg-gray-100'}>
              <Settings className="h-4 w-4" />
            </Button>
            
            <Button variant="ghost" size="sm" onClick={() => setIsDarkMode(!isDarkMode)} className={isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-700' : 'text-gray-600 hover:bg-gray-100'}>
              {isDarkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </Button>
            
            <Button onClick={handleNewChat} className={`${isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
          ...
          minHeight: 'auto'
            ...
            return <motion.div key={card.id} variants={itemVariants} whileHover={{
              ...
              ease: "easeOut"
                    ...
                    <Card className={`overflow-hidden shadow-sm border cursor-pointer flex flex-col w-full h-full transition-all duration-200 ease-out ${selectedCardId === card.id ? isDarkMode ? 'ring-2 ring-blue-500/80 bg-slate-800 border-blue-500/50 shadow-lg' : 'ring-2 ring-blue-500/80 bg-blue-50 border-blue-500/50 shadow-lg' : isDarkMode ? 'bg-slate-800/50 border-slate-700 hover:bg-slate-800 hover:shadow-md' : 'bg-white border-gray-200 hover:bg-gray-50 hover:shadow-md'}`}>
                      <CardHeader className={`flex flex-row items-center justify-between p-4 border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
                        <div className="flex items-center flex-1">
                          <span className="mr-2">{card.icon}</span>
                          {cardIsLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-500 mr-2"></div>}
                      ...
                      <CardContent className="p-4 flex-1 flex flex-col my-[217px]">
                        {/* Pinned Messages Section */}
                        {pinnedMessages.length > 0 && <div className={`mb-4 p-3 rounded-lg border-l-4 border-yellow-500 ${isDarkMode ? 'bg-yellow-500/10 border-yellow-500/50' : 'bg-yellow-50 border-yellow-500'}`}>
                        ...
                        message,
                        index
                      }) => <div key={index} className={`p-2 rounded cursor-pointer transition-colors duration-200 ${isDarkMode ? 'bg-slate-800/50 hover:bg-slate-700/50 text-slate-200' : 'bg-white hover:bg-gray-50 text-gray-800'}`} onClick={e => {
                        e.stopPropagation();
                        ...
                        messageRefs.current[card.id][idx] = el;
                                ...
                                <div className={`inline-block p-3 rounded-lg max-w-[85%] relative transition-all duration-200 ease-out ${message.isUser ? 'bg-blue-600 text-white ml-auto' : isDarkMode ? 'bg-slate-700 text-white border border-slate-600' : 'bg-gray-100 text-gray-900 border border-gray-200'} ${isMessagePinned(message) ? 'ring-2 ring-yellow-500/60' : ''}`}>
                                  {isMessagePinned(message) && <Pin className="absolute -top-2 -right-2 h-4 w-4 text-yellow-500 bg-slate-800 rounded-full p-0.5" />}
                                  {message.content}
                                </div>
                                {!message.isUser && <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                      <Button variant="ghost" size="sm" className={`opacity-0 group-hover:opacity-100 transition-opacity duration-200 ml-2 ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-700' : 'text-gray-600 hover:bg-gray-100'}`} onClick={e => e.stopPropagation()}>
                                        <MoreVertical className="h-3 w-3" />
                                      </Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent className={isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}>
                                      {isMessagePinned(message) ? <DropdownMenuItem onClick={() => handleUnpinMessage(card.id, idx)} className={isDarkMode ? 'text-slate-300 hover:text-white hover:bg-slate-700' : 'text-gray-700 hover:bg-gray-100'}>
                                          <Pin className="h-4 w-4 mr-2" />
                                          Unpin message
                                        </DropdownMenuItem> : <DropdownMenuItem onClick={() => handlePinMessage(card.id, idx)} className={isDarkMode ? 'text-slate-300 hover:text-white hover:bg-slate-700' : 'text-gray-700 hover:bg-gray-100'}>
                                          <Pin className="h-4 w-4 mr-2" />
                                          Pin message
                                        </DropdownMenuItem>}
                                    </DropdownMenuContent>
        ...
        <AnimatePresence>
          {minimizedCardsList.length > 0 && <motion.div className="mt-8 mb-6" initial="hidden" animate="visible" exit="exit">
              ...
              return <motion.div key={`minimized-${card.id}`} variants={minimizedTabVariants} initial="hidden" animate="visible" exit="exit" layout layoutId={`minimized-${card.id}`} className="inline-block">
                      <Card className={`overflow-hidden shadow-sm border cursor-pointer transition-all duration-200 ease-out hover:shadow-md min-w-[200px] max-w-[280px] ${selectedCardId === card.id ? isDarkMode ? 'ring-2 ring-blue-500/80 bg-slate-800 border-blue-500/50' : 'ring-2 ring-blue-500/80 bg-blue-50 border-blue-500/50' : isDarkMode ? 'bg-slate-800/70 border-slate-700 hover:bg-slate-800' : 'bg-white border-gray-200 hover:bg-gray-50'}`} onClick={() => handleSelectCard(card.id)}>
                        <CardHeader className={`flex flex-row items-center justify-between p-3 ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
                          <div className="flex items-center flex-1 min-w-0">
                            <span className="mr-2 flex-shrink-0">{card.icon}</span>
                            {cardIsLoading && <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-blue-500 mr-2 flex-shrink-0"></div>}
        ...
        ease: "easeOut"
          ...
          <div className={`border rounded-xl p-4 shadow-sm transition-all duration-200 ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center space-x-2">
              <Input className={`flex-1 border-0 bg-transparent focus:ring-0 h-12 transition-all duration-200 ${isDarkMode ? 'text-white placeholder:text-gray-400' : 'text-gray-900'} ${!selectedCardId || currentCardLoading ? 'opacity-70' : ''}`} placeholder={selectedCardId ? currentCardLoading ? `"${chatCards.find(card => card.id === selectedCardId)?.title}" is thinking...` : `Type in "${chatCards.find(card => card.id === selectedCardId)?.title}" chat...` : "Select a chat first..."} value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && !currentCardLoading && handleSendMessage()} disabled={!selectedCardId || currentCardLoading} />
              <Button size="lg" variant="outline" onClick={handleAddImage} disabled={!selectedCardId || currentCardLoading} className={`h-12 transition-all duration-200 ${isDarkMode ? 'border-slate-600 text-green-400 hover:text-green-300 hover:bg-slate-700/50' : 'border-gray-300 text-green-600 hover:text-green-700 hover:bg-green-50'} ${!selectedCardId || currentCardLoading ? 'opacity-70' : ''}`}>
                <Image className="h-5 w-5" />
              </Button>
              <Button size="lg" onClick={handleSendMessage} disabled={!selectedCardId || currentCardLoading} className={`bg-blue-600 hover:bg-blue-700 text-white h-12 transition-all duration-200 ${!selectedCardId || currentCardLoading ? 'opacity-70' : ''}`}>
        ...
        ease: "easeOut"
          ...
          {['#assignment', '#travel', '#food', '#ideas'].map((tag, idx) => <Button key={tag} variant="outline" className={`transition-all duration-200 ${isDarkMode ? 'bg-slate-800/50 text-slate-300 border-slate-600 hover:text-white hover:bg-slate-700/50' : 'bg-gray-50 text-gray-700 border-gray-300 hover:bg-gray-100'}`}>
              <span className={`w-2 h-2 rounded-full mr-2 ${idx === 0 ? 'bg-blue-500' : idx === 1 ? 'bg-green-500' : idx === 2 ? 'bg-purple-500' : 'bg-yellow-500'}`}></span>
              {tag}
            </Button>)}
        </motion.div>
        ...
        ease: "easeOut"
          ...
          {['Tasks Completed', 'Premium Credits', 'Response Time', 'Satisfaction'].map((stat, i) => <motion.div key={stat} className={`rounded-xl p-4 text-center border transition-all duration-200 hover:shadow-md ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-gray-200'}`} whileHover={{
          scale: 1.02,
          transition: {
            duration: 0.2,
            ease: "easeOut"
      ...
      <Dialog open={pinDialogOpen} onOpenChange={setPinDialogOpen}>
        <DialogContent className={isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Button onClick={() => handlePinWithDuration('8hours')} variant="outline" className={`w-full justify-start transition-all duration-200 ${isDarkMode ? 'border-slate-600 text-orange-400 hover:text-orange-300 hover:bg-orange-500/10 hover:border-orange-400' : 'border-gray-300 text-orange-600 hover:text-orange-700 hover:bg-orange-50 hover:border-orange-400'}`}>
                ⏰ 8 Hours
              </Button>
              <Button onClick={() => handlePinWithDuration('1week')} variant="outline" className={`w-full justify-start transition-all duration-200 ${isDarkMode ? 'border-slate-600 text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 hover:border-purple-400' : 'border-gray-300 text-purple-600 hover:text-purple-700 hover:bg-purple-50 hover:border-purple-400'}`}>
                📅 1 Week
              </Button>
              <Button onClick={() => handlePinWithDuration('forever')} variant="outline" className={`w-full justify-start transition-all duration-200 ${isDarkMode ? 'border-slate-600 text-yellow-400 hover:text-yellow-300 hover:bg-yellow-500/10 hover:border-yellow-400' : 'border-gray-300 text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50 hover:border-yellow-400'}`}>
                ♾️ Forever
              </Button>
            </div>
          </div>
